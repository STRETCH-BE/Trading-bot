#!/usr/bin/env python3
"""Stage 4a gate applied to STRATEGY #2: volatility-targeted trend.

Preregistration amendment 2 (commit 51da0b3) fixed the spec before any result:
grid of exactly 12 combinations, costs and boundaries unchanged from the
strategy #1 protocol.
...
def _voltrend_signal(candles, params):
    return voltrend(candles, params)


def _search_job(args):
    tag, widx, is_df, pair = args
    pts = search_window(is_df, COMBOS, CONFIG, pair, signal_factory=_voltrend_signal)
            ...
            "windows": [
                {
                    ...
                    "trend": o.params.trend_lookback,
                    "vol": o.params.vol_lookback,
                    "tvol": o.params.target_vol,
...
def main() -> int:
    ...
    print(f"stage 4a STRATEGY #2 voltrend | commit {git_hash} | shuffle seeds {SHUFFLE_SEEDS} | "
    ...
    results: dict = {
        "meta": {
            "stage": "4a_strategy2_voltrend",
            "timestamp_utc": stamp,
            "git_hash": git_hash,
            "shuffle_seeds": SHUFFLE_SEEDS,
            "grid_size": len(COMBOS),
            "grid": {"trend_lookback": [50, 100, 200], "vol_lookback": [20, 60],
    ...
    out_path = Path("results") / f"4a_strategy2_voltrend_{stamp}_{git_hash}.json"
